...
from scene.blur_kernel import BlurKernel_mask
...
class GaussianModel:
    ...
    def __init__(self, cfg,num_img):
        ...
        self.mlp_rgb=BlurKernel_mask(num_img=num_img).to("cuda")
    ...
    def clone(self):
        ...
        cloned = GaussianModel(self.cfg,self.num_img)
    ...
    def get_covariance(self, scaling_modifier = 1,scale_precomp=None,rotation_rotation_precomp=None):
        ...
        if rotation_rotation_precomp is not None:
            return self.covariance_activation(scale_precomp, scaling_modifier, rotation_rotation_precomp)
        elif hasattr(self, 'rotation_precomp') and scale_precomp is not None:
            return self.covariance_activation(scale_precomp, scaling_modifier, self.rotation_precomp)
        elif hasattr(self, 'rotation_precomp'):
            return self.covariance_activation(self.get_scaling, scaling_modifier, self.rotation_precomp)
        # print("rotation_precomp not found")
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale=1.):
        ...
        scales = torch.log(torch.sqrt(dist2+1e-6))[...,None].repeat(1, 3)
    ...
    def training_setup(self, training_args):
        ...
        # l+=[{'params':self.gtnet.parameters(),'lr':self.cfg.gtnet.gtnet_lr,'name':'gtnet'}]]
        lr_mlp=5e-4
        for p in self.mlp_rgb.parameters():
            l += [{'params': [p], 'lr': lr_mlp,'name': 'mask_mlp'}]
    ...
    def add_densification_stats(self, viewspace_point_tensor, update_filter,d=1.0):
        ...
        # if type(update_filter)==list:
            for i in range(len(viewspace_point_tensor)):
            # for i in range(len(update_filter)):
