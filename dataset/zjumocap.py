...
class ZJUMoCapDataset(Dataset):
    def __init__(self, cfg, split='train'):
        ...
        if split == 'predict' or cfg.freeview:
            ...
            for cam_idx, cam_name in enumerate(cam_names):
                ...
                ## modified by lxr
                if 'motion' not in cam_name:
                    # cam_dir_mask=cam_dir.replace('motion','')
                    mask_files = sorted(glob.glob(os.path.join(cam_dir, '*.png')))[frame_slice]

                for d_idx, f_idx in enumerate(frames):
                    model_file = model_files[d_idx]
                    number=model_file.split('/')[-1].split('.')[0]
                    img_file=os.path.join(cam_dir,number+'.jpg')
                    # img_file = img_files[d_idx]
                    # if 'motion' not in cam_name:
                    #     mask_file = mask_files[d_idx]
                    # else:
                        # mask_file=img_file.replace('jpg','png').replace('_motion' ,'')
                        # motion_name=img_file.split('/')[-2]
                        # new_name=motion_name.split('_')[0]
                        # mask_file=img_file.replace('jpg','png').replace(motion_name ,new_name)
                    mask_file=img_file.replace('jpg','png').replace('_motion' ,'')
                    model_file = model_files[d_idx]
    ...
    def getitem(self, idx, data_dict=None):
        ...
        ##modified by lxr
        if 'motion' in cam_name:
            newname=cam_name.split('_')[0]
            # cam_name = cam_name.replace('_motion','')
            cam_name = cam_name.replace(cam_name,newname)
        ...
        # original_image = torch.from_numpy(original_image).permute(2, 0, 1).float()
