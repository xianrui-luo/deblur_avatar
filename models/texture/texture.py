...
from models.network_utils import VanillaCondMLP, HierarchicalPoseEncoder, HashGrid
...
##modified
        # self.hashgrid = HashGrid(cfg.non_rigid.hashgrid)
        # self.aabb = metadata['aabb']
        # self.pose_encoder = HierarchicalPoseEncoder(**cfg.non_rigid.pose_encoder)
        ...
        # self.mlp_delta=VanillaCondMLP(self.hashgrid.n_output_dims, d_cond, (3+3+4)*num_moments, cfg.non_rigid.mlp)

        # ##predict alpha for each point
        self.alpha_mlp=VanillaCondMLP(d_in, 0, 1, cfg.mlp)
    ...
    def forward(self, gaussians, camera,iteration):
        ...
        #     inp = self.compose_input(gaussians[i], camera[i])
        #     output = self.mlp(inp)
        #     color = self.color_activation(output)
        #     color_list.append(color)
        # if len(camera)>1:
        #     length=len(camera)
        #     camera_use=camera[(length-1)//2]
        # else:   
        #     camera_use=camera[0]
        camera_use=camera[0]
        inp=self.compose_input(gaussians, camera_use)
        output = self.mlp(inp)
        color = self.color_activation(output)

        alpha = self.alpha_mlp(inp)

        # if iteration>self.cfg.non_rigid_iteration:
        #     camera_list=camera_list[0]

        # if iteration>self.cfg.non_rigid_iteration:
        #     # xyz = gaussians[0].get_xyz
        #     # xyz_norm = self.aabb.normalize(xyz, sym=True)
        #     # feature = self.hashgrid(xyz_norm)
        #     # pose_feat = self.pose_encoder(camera[0].rots, camera[0].Jtrs)
        #     # # d_cond = self.pose_encoder.n_output_dims
        #     # if self.latent_dim > 0:
        #     #     frame_idx = camera[0].frame_id
        #     #     if frame_idx not in self.frame_dict:
        #     #         latent_idx = len(self.frame_dict) - 1
        #     #     else:
        #     #         latent_idx = self.frame_dict[frame_idx]
        ...
        #     # deltas = self.mlp_delta(feature, cond=pose_feat)
        #     # scales_delta = deltas[:, :3*4]
        #     # rotations_delta = deltas[:, 3*4:3*4+4*4]
        #     # pos_delta = deltas[:, 3*4+4*4:]
            

        #     ##smaller network
        #     scales_delta = self.s(inp)
        #     rotations_delta = self.r(inp)
        #     pos_delta = self.p(inp)
        #     scales_delta = scales_delta.view(-1, 3, 4)
        #     rotations_delta = rotations_delta.view(-1, 4, 4)
        #     pos_delta = pos_delta.view(-1, 3,4)
        #     return color_list,scales_delta, rotations_delta, pos_delta

        return color

class ColorMLP_nonrigid(ColorPrecompute):
    def __init__(self, cfg, metadata):
        super().__init__(cfg, metadata)
        d_in = cfg.feature_dim
...
def get_texture_nonrigid(cfg, metadata):
    name = cfg.name
    model_dict = {
        # "sh2rgb": SH2RGB,
        "mlp": ColorMLP_nonrigid,
    }
    return model_dict[name](cfg, metadata)