...
class HashGridwithMLP_motion(NonRigidDeform):
    def __init__(self, cfg, metadata):
        super().__init__(cfg)
        self.pose_encoder = HierarchicalPoseEncoder(**cfg.pose_encoder)
        d_cond = self.pose_encoder.n_output_dims
    ...
    def forward(self, gaussians, iteration, camera, compute_loss=True,delay_flag=False):
        if len(camera)!=1:

            flag_camera=camera[(len(camera)-1)//2]
        else:
            flag_camera=camera[0]
        ...
        if iteration < self.delay:
            ...
            delta_scale=torch.zeros(gaussians.get_xyz.shape[0], 3).cuda().detach()
            delta_rot=torch.zeros(gaussians.get_xyz.shape[0], 4).cuda().detach()
            delta_xyz=torch.zeros(gaussians.get_xyz.shape[0], 3).cuda().detach()
            return deformed_gaussians, {},delta_xyz,delta_scale,delta_rot
            

        rots = flag_camera.rots
        Jtrs = flag_camera.Jtrs
        pose_feat = self.pose_encoder(rots, Jtrs)
        ...
        deformed_gaussians._xyz = gaussians._xyz + delta_xyz
         ##small delta
        # deformed_gaussians._xyz = gaussians._xyz + delta_xyz*lambda_p
        ...
        loss_reg = {}
        return deformed_gaussians, loss_reg, delta_xyz,delta_scale,delta_rot
