...
class SMPLNN(RigidDeform):
    ...
    def forward(self, gaussians, iteration, camera):
        ...
        ## why no rotation The commented-out lines suggest that the code originally intended to
        #  convert the rotations back to quaternions, but this step is currently skipped.
...
class SkinningField_2(RigidDeform):
    def __init__(self, cfg, metadata):
        super().__init__(cfg)
    ...
    def forward(self, gaussians, iteration, camera,camera_original,pos_delta,scale_delta,rotation_delta):
        # deformed_gaussians_out = []
        rotation_bar_list=[]
        x_bar_list=[]
        lambda_p=0.01
        lambda_s=0.01
        ...
        for i in range(len(camera)):
            tfs = camera[i].bone_transforms

            # gaussians_copy=gaussians.clone()
            # # gaussians_copy._xyz = gaussians_copy._xyz+pos_delta*lambda_p
            # gaussians_copy._xyz = gaussians_copy._xyz+pos_delta
            # q1 = rotation_delta
            # # q1[0] = 1.  # [1,0,0,0] represents identity rotation
            # q1[:,0] = 1.  # [1,0,0,0] represents identity rotation
            # # q1 = torch.clamp(lambda_s * q1 + (1-lambda_s), min=1.0, max=max_clamp)
            # # delta_rot = delta_rot[1:]
            # q2 = gaussians_copy._rotation
            # # deformed_gaussians._rotation = quaternion_multiply(q1, q2)
            # gaussians_copy._rotation = tf.quaternion_multiply(q1, q2)
            ...
            # if i==(len(camera)-1)//2 or len(camera)==1:
            #     deformed_gaussians_out=deformed_gaussians
            #     deformed_gaussians_out.set_fwd_transform(T_fwd.detach())
            ...
            rotation_bar = torch.matmul(T_fwd[:, :3, :3], rotation_hat)
            # setattr(deformed_gaussians, 'rotation_precomp', rotation_bar)
            rotation_bar_list.append(rotation_bar)
            x_bar_list.append(x_bar)

        tfs=camera_original.bone_transforms
        # deformed_gaussians = gaussians_copy
        # deformed_gaussians._xyz=deformed_gaussians._xyz+pos_delta
        # q1 = rotation_delta
        # q1[:,0] = 1.  # [1,0,0,0] represents identity rotation
        # q2 = deformed_gaussians._rotation
        ...
        return deformed_gaussians_out,x_bar_list,rotation_bar_list
...
class SkinningField_nonrigid(RigidDeform):
    def __init__(self, cfg, metadata):
        super().__init__(cfg)
        self.smpl_verts = metadata["smpl_verts"]
        self.skinning_weights = metadata["skinning_weights"]
    ...
    def forward(self, gaussians, iteration, camera,camera_original):
        # deformed_gaussians_out = []
        rotation_bar_list=[]
        x_bar_list=[]
        lambda_p=0.01
...
def get_rigid_deform(cfg, metadata):
    ...
    model_dict = {
        "identity": Identity,
        "smpl_nn": SMPLNN,
        "motion": SkinningField_2,
        "nonrigid":SkinningField_nonrigid,
    }
