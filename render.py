...
# # ZJU-MoCap
# python render.py mode=test dataset.test_mode=view dataset=zjumocap_377_mono_motion
# # PeopleSnapshot
# python render.py mode=test dataset.test_mode=pose pose_correction=none dataset=ps_female_3

def predict(config):
    with torch.set_grad_enabled(False):
        gaussians = GaussianModel(config.model.gaussian,1)
        ...
        for idx in trange(len(scene.test_dataset), desc="Rendering progress"):
            ...
            render_pkg = render_pose(view, config.opt.iterations, scene, config.pipeline, background,
            # render_pkg = render_pose_test(view, config.opt.iterations, scene, config.pipeline, background,
                                compute_loss=False, return_opacity=False)
            iter_end.record()
            torch.cuda.synchronize()
            elapsed = iter_start.elapsed_time(iter_end)

            rendering = render_pkg["render_original"]
            rgb=render_pkg["render"]

            wandb_img = [wandb.Image(rendering[None], caption='render_{}'.format(view.image_name)),]
            wandb.log({'test_images': wandb_img})
            wandb_img = [wandb.Image(rgb[None], caption='render_blurred_{}'.format(view.image_name)),]
            wandb.log({'test_images': wandb_img})

            torchvision.utils.save_image(rendering, os.path.join(render_path, f"render_{view.image_name}.png"))
            torchvision.utils.save_image(rgb, os.path.join(render_path, f"render_blurred_{view.image_name}.png"))
...
def test(config):
    with torch.no_grad():
        ...
        if load_ckpt is None:
            load_ckpt = os.path.join(scene.save_dir, "ckpt" + str(config.opt.iterations) + ".pth")
            # load_ckpt = os.path.join(scene.save_dir, "ckpt" + '10000' + ".pth")
        scene.load_checkpoint(load_ckpt)
        ...
        for idx in trange(len(scene.test_dataset), desc="Rendering progress"):
            ...
            rendering = render_pkg["render_original"]
            blurred=render_pkg["render"]

            gt = view.original_image[:3, :, :]

            wandb_img = [wandb.Image(rendering[None], caption='render_{}'.format(view.image_name)),
                            wandb.Image(blurred[None], caption='render_blurred_{}'.format(view.image_name)),
                         wandb.Image(gt[None], caption='gt_{}'.format(view.image_name))]

            wandb.log({'test_images': wandb_img})

            torchvision.utils.save_image(rendering, os.path.join(render_path, f"render_{view.image_name}.png"))
            torchvision.utils.save_image(blurred, os.path.join(render_path, f"render_blurred_{view.image_name}.png"))
        ...
        wandb.log({'metrics/psnr': _psnr,
                   ...
                   'metrics/time': _time})
        print(f"PSNR: {_psnr}, SSIM: {_ssim}, LPIPS: {_lpips}, Time: {_time}")
        np.savez(os.path.join(config.exp_dir, config.suffix, 'results.npz'),
